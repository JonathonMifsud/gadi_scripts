...
############################################################################################################
# Script Name: blastx_custom_runner.sh
# Description: Submits one or more DIAMOND blastx jobs for user-specified inputs and database on Gadi.
...
Usage:
  $0 -i input_fasta -d diamond_db.dmnd [options]
  $0 -l list_of_fastas.txt -d diamond_db.dmnd [options]

Description:
  Launches DIAMOND blastx jobs on Gadi. Supports both:
...
Required:
  -i        Input nucleotide FASTA file (for single job) OR
...
Examples:
  Run a single job:
    $0 -i /scratch/fo27/user/my.fasta -d /g/data/fo27/dbs/nr.dmnd

  Run multiple jobs in parallel:
    $0 -l inputs.txt -d /scratch/fo27/dbs/nr.202405.dmnd -o /scratch/fo27/user/results -n 4

  Override DIAMOND parameters:
    $0 -i my.fa -d db.dmnd -x "--ultra-sensitive -e 1e-5 -k 5"
...
echo -e "\n🎉 All blastx jobs have been submitted."
