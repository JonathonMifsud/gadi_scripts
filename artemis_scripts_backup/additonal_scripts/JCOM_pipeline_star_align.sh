...
qsub -J "$jPhrase" \
    -o "/project/$root_project/$project/logs/star_custom_^array_index^_$project_$(date '+%Y%m%d')_stdout.txt" \
    -e "/project/$root_project/$project/logs/star_custom_^array_index^_$project_$(date '+%Y%m%d')_stderr.txt" \
    -v "project=$project,mapping_spreadsheet=$mapping_spreadsheet,root_project=$root_project" \
    -q "$queue" \
    -l "$job_time" \
    -P "$root_project" \
    /project/"$root_project"/"$project"/scripts/"$project"_star_align.pbs
